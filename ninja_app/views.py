...
def money(request):
    ...
    if 'cont' in request.session:
        if request.POST['ninja'] == 'Farm':
            ...
            diccionario = { 'texto': f'¡¡¡Ganaste {a} monedas de oro!!!', 'color': 'verde', 'date': strftime("%H:%M %p", localtime()) }
        ...
        if request.POST['ninja'] == 'Casino':
            ...
            if a == 1:
                ...
                diccionario = { 'texto': f'¡¡¡Ganaste {b} monedas de oro!!!', 'color': 'verde', 'date': strftime("%H:%M %p", localtime()) }
                request.session['list'].append(diccionario)
            else:
                b = random.randint(50,60)
                request.session['cont'] -= b
                diccionario = { 'texto': f'¡¡¡Perdiste {b} monedas de oro!!!', 'color': 'rojo', 'date': strftime("%H:%M %p", localtime()) }
                request.session['list'].append(diccionario)
